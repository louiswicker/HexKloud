...
c
c     remove large time step values from ru1, ru2, ru3, rw1, and omega 
c
      do j=1,ny
         do i=iper,nx
            do k=1,nz1
...
c
     ...
     &                         -(rw1 (k  ,i,j  )+ww (k  ,i,j  ))
     &                            *(t(k  ,i,j  )+t  (km1,i,j  )))
cccd                  div(k,i,j)=  div(k,i,j)/t(k,i,j)

               end do
            end do
         end do
c
c        time step for u1 and u3 equations
c
         if(iper.eq.0)  then
            do j=1,ny
               do k=1,nz1
         ...
         if(.not.test1) then
            ...
            do j=1,ny              
               ...
               do i=1,nx              
                  ...
                  if(iper*i.eq.nx) ip1 = 2
c
c                 explicit portion of rr1 time step
c
                  do k=1,nz1
...
c
c                 explicit portion of rt1 time step
c
                  do k=1,nz1
...
c
c                 explicit portion of rw1 time step
c
                  do k=2,nz1
...
c
c           implicit solution for rw1, rr1, and rt1 
c
            do j=1,ny
               do i=1,nx
                  do k=2,nz1
...
c        end of small time step

      end do
c
c     add large time step rw back into rw1 
c
      do j=1,ny
		 jv1 = j-1
		 if(jper*j.eq.1)  jv1 = ny1
