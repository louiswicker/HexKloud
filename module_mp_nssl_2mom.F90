      ...
      IF ( irenuc >= 5 ) THEN
        ...
        IF ( present( nssl_ccn_on ) ) THEN
          IF ( .not. nssl_ccn_on ) THEN
           write(0,*) 'NSSL_MP Error: Must have nssl_ccn_on=1/true for irenuc >= 5!'
        STOP
          ENDIF
        ENDIF
      ENDIF
...
! 20130903 acd_mb_washout end
 ...
 !Subroutine arguments:
      ...
      real, dimension(kms:kme, ims:ime, jms:jme), intent(inout)::                        &
                            qv,qc,qr,qs,qh
      ! tt is air temperature -- used by CCPP instead of th (theta)
      real, dimension(kms:kme, ims:ime, jms:jme), optional, intent(inout)::                       &
                              th, tt,                                                   &
                              zrw, zhw, zhl,                                            &
                              qsw, qhw, qhlw,                                           &
      ...
      real, dimension(kms:kme, ims:ime, jms:jme), optional, intent(inout):: dbz, vzf, cn, cna, cni, cnuf
      real, dimension(kms:kme, ims:ime, jms:jme), optional, intent(inout):: cn_nu, cn_ac, cn_co, cinp, cna_co, cna_nu
...
!      real, dimension(kms:kme, ims:ime, jms:jme), optional, intent(inout)::rscghis_3d
      ...
      real, dimension(kms:kme, ims:ime, jms:jme),  optional, intent(in) :: elecz ! elecsave = Ez
      real, dimension(kms:kme, ims:ime, jms:jme, 2),optional, intent(inout) :: scion
      real, dimension(kms:kme, ims:ime, jms:jme), intent(in)::  p,w,dz,dn

      real, dimension(kms:kme, ims:ime, jms:jme), intent(in)::  pii
      real, dimension(kms:kme, ims:ime, jms:jme), optional, intent(inout)::   &
                              ssat3d, ssati,              &
                              pcc2, pre2, depsubr,      &
                              mnucf2, melr2, ctr2,     &
...
!                              vtcloud, vtrain, vtsnow, vtgraupel, vthail               
       ...
       real, dimension(kms:kme, ims:ime, jms:jme), optional, intent(inout) :: axtra
...
!     for cm1, set nproctot=44 (or as needed) to get domain total rates
      ...
      REAL, DIMENSION(kms:kme, ims:ime, jms:jme), optional, INTENT(INOUT)::  re_cloud, re_ice, re_snow, &
                                                                   re_rain, re_graup, re_hail
      REAL, DIMENSION(kms:kme, ims:ime, jms:jme), optional, INTENT(IN):: tkediss
...
!   REAL, DIMENSION(kms:kme, ims:ime, jms:jme), optional,INTENT(INOUT):: qndrop
...
! 20130903 acd_ck_washout end
   REAL, DIMENSION(kms:kme, ims:ime, jms:jme), optional,INTENT(INOUT)::  rainprod, evapprod
...
! mu : air mass in column
   REAL, DIMENSION(kms:kme, ims:ime, jms:jme), optional, INTENT(IN):: qrcuten, qscuten, qicuten, qccuten
     ...
     ! but kept for backwards compatibility with earlier versions
      ...
      IF ( invertccn .and.  flag_ccn ) THEN ! hack for WRF to convert activated ccn to unactivated, then do not have to 
        ...
        DO jy = jts,jte
         DO kz = kts,kte
           DO ix = its,ite
             cn(kz,ix,jy) = Max( 0.0, qccn - cn(kz,ix,jy) )
           ENDDO
         ENDDO
       ENDDO
...
!       write(0,*) 'set cnuf (invertccn)'
        DO jy = jts,jte
         DO kz = kts,kte
          DO ix = its,ite
             cnuf(kz,ix,jy) = qccnuf - cnuf(kz,ix,jy)
          ENDDO
         ENDDO
        ENDDO
       ENDIF
...
!     write(0,*) 'N2M: load an, jy,lccn = ',jy,lccn,qccn
   ...
   ! copy from 3D array to 2D slab
       ...
       DO kz = kts,kte
        DO ix = its,ite
          an(ix,1,kz,lt) = th(kz,ix,jy)
          an(ix,1,kz,lv)   = qv(kz,ix,jy)
          an(ix,1,kz,lc)   = qc(kz,ix,jy)
          an(ix,1,kz,lr)   = qr(kz,ix,jy)
          IF ( flag_qi ) THEN
            an(ix,1,kz,li)   = qi(kz,ix,jy)
          ELSE
            an(ix,1,kz,li) = 0.0
          ENDIF
          an(ix,1,kz,ls)   = qs(kz,ix,jy)
          an(ix,1,kz,lh)   = qh(kz,ix,jy)
          IF ( lhl > 1 ) an(ix,1,kz,lhl)  = qhl(kz,ix,jy)
          IF ( lccn > 1 ) THEN
           IF ( is_aerosol_aware .and. flag_qnwfa ) THEN
            ! 
           ELSEIF ( flag_ccn ) THEN
             IF ( lccna > 1 .and. .not. ( present( cna ) .and. f_cnatmp ) ) THEN
               an(ix,1,kz,lccna) = cn(kz,ix,jy)
               an(ix,1,kz,lccn) = qccn ! cn(kz,ix,jy)
             ELSE
               an(ix,1,kz,lccn) = cn(kz,ix,jy)
             ENDIF
             IF ( i_uf_or_ccn > 0 .and. lccnuf > 1 ) THEN ! UF ccn are extra regular ccn
               an(ix,1,kz,lccn) = an(ix,1,kz,lccn) + cnuf(kz,ix,jy)
             ENDIF
           ELSE
            IF ( lccna == 0 .and. ( .not. f_cnatmp ) ) THEN
              an(ix,1,kz,lccn) = qccn - ccw(kz,ix,jy)
            ELSE
          ...
          IF ( lccnuf > 0 .and. flag_cnuf ) THEN
            IF ( i_uf_or_ccn == 0 ) THEN ! UF are UF
              an(ix,1,kz,lccnuf) = Max(0.0, cnuf(kz,ix,jy) )
          ...
          IF ( lccna > 1 ) THEN
            IF ( present( cna ) .and. f_cnatmp ) THEN
              an(ix,1,kz,lccna) = Max(0.0, cna(kz,ix,jy) )
            ENDIF
          ENDIF

          IF ( lcina > 1 ) THEN
            IF ( present( cni ) .and. f_cinatmp ) THEN
              an(ix,1,kz,lcina) = cni(kz,ix,jy)
            ENDIF
          ENDIF
          
          IF ( ipconc >= 5 ) THEN
             an(ix,1,kz,lnc)  = ccw(kz,ix,jy)
          IF ( constccw > 0.0 ) THEN
            an(ix,1,kz,lnc)  = constccw
          ENDIF
          an(ix,1,kz,lnr)  = crw(kz,ix,jy)
          IF ( present( cci ) ) THEN
            an(ix,1,kz,lni)  = cci(kz,ix,jy)
          ELSE
            an(ix,1,kz,lni) = 0.0
          ENDIF
          an(ix,1,kz,lns)  = csw(kz,ix,jy)
          ...
          IF ( ipconc >= 6 ) THEN
            IF ( lzr > 0 )  an(ix,1,kz,lzr)  = zrw(kz,ix,jy)*zscale
            IF ( lzh > 0 )  an(ix,1,kz,lzh)  = zhw(kz,ix,jy)*zscale
            IF ( lzhl > 0 ) an(ix,1,kz,lzhl) = zhl(kz,ix,jy)*zscale
          ENDIF
       ...
       DO kz = kts,kte
        DO ix = its,ite
          ...
          t0(ix,1,kz) = th(kz,ix,jy)*pii(kz,ix,jy) ! temperature (Kelvin)
          t00(ix,1,kz) = 380.0/p(kz,ix,jy)
          t77(ix,1,kz) = pii(kz,ix,jy)
          dbz2d(ix,1,kz) = 0.0
          vzf2d(ix,1,kz) = 0.0
        ENDDO
       ENDDO
      ...
      DO loopcnt = 1,loopmax
       ...
       DO kz = kts,kte
        DO ix = its,ite
          ...
          pn(ix,1,kz) = p(kz,ix,jy)
          wn(ix,1,kz) = w(kz,ix,jy)
          dn1(ix,1,kz) = dn(kz,ix,jy)
!          wmax = Max(wmax,wn(ix,1,kz))
          dz2d(ix,1,kz) = dz(kz,ix,jy)
          dz2dinv(ix,1,kz) = 1./dz(kz,ix,jy)
    ...
    IF ( loopcnt == 1 ) THEN
     DO il = lnb,na
       IF ( denscale(il) == 1 ) THEN
         DO kz = kts,kte
          DO ix = its,ite
           an(ix,1,kz,il) = an(ix,1,kz,il)*dn1(ix,1,kz) ! dn(kz,ix,jy)
          ENDDO
         ENDDO
       ENDIF
     ENDDO ! il
      ...
      IF ( present(cu_used) .and.         &
       ...
       DO kz = kts,kte
        DO ix = its,ite
         ...
         IF ( present( qrcuten ) ) ancuten(ix,1,kz,lr) = dtp*qrcuten(kz,ix,jy)
         IF ( present( qscuten ) ) ancuten(ix,1,kz,ls) = dtp*qscuten(kz,ix,jy)
         IF ( present( qicuten ) ) ancuten(ix,1,kz,li) = dtp*qicuten(kz,ix,jy)
         IF ( present( qccuten ) ) ancuten(ix,1,kz,lc) = dtp*qccuten(kz,ix,jy)
...
!            pcc2(kz,ix,jy)    = axtra2d(ix,1,kz,1)
          ENDDO
         ENDDO
     ENDIF
...
!          c1 = t00(ix,1,kz)*tabqvs(ltemq)
          ...
          IF ( c1 > 0. ) THEN
            ssat3d(kz,ix,jy) = 100.*(an(ix,1,kz,lv)/c1 - 1.0)  ! from "new" values
          ENDIF
          ...
          IF ( f_ssati ) THEN
           ...
           ssati(kz,ix,jy) = 100.*(an(ix,1,kz,lv)/t9s - 1.0) !  Min(t8s,max(an(ix,1,kz,lv),0.0))/t9s  ! qv/qvi
          ENDIF
          ENDIF
     ...
     &    dbz2d,dn1,nz,cnoh,rho_qh,ipconc,kediagloc, 0)
      ENDIF ! .false.
       ...
       DO kz = kts,kediagloc ! kte
        DO ix = its,ite
         dbz(kz,ix,jy) = dbz2d(ix,1,kz)
         IF ( present( vzf ) ) THEN
           vzf(kz,ix,jy) = vzf2d(ix,1,kz)
           IF ( dbz2d(ix,1,kz) <= 0.0 ) THEN
             vzf(kz,ix,jy) = 0.0
           ELSEIF ( dbz2d(ix,1,kz) <= 15.0 ) THEN
             refl = 10**(0.1*dbz2d(ix,1,kz))
             vzf(kz,ix,jy) = Min( vzf2d(ix,1,kz), 2.6 * Max(0.0,refl)**0.107 * (1.2/dn1(ix,1,kz))**0.4 )
           ENDIF
         ENDIF
...
! Following Greg Thompson, calculation for effective radii. Used by RRTMG LW/SW schemes if enabled in module_physics_init.F
      IF ( present( has_reqc ).and. present( has_reqi ) .and. present( has_reqs ) .and.  &
       ...
       IF ( has_reqc.ne.0 .or. has_reqi.ne.0 .or. has_reqs.ne.0) THEN
         DO kz = kts,kte
          DO ix = its,ite
             re_cloud(kz,ix,jy)  = 2.51E-6
             re_ice(kz,ix,jy)    = 10.01E-6
             re_snow(kz,ix,jy)   = 25.E-6
             t1(ix,1,kz) = 2.51E-6
             t2(ix,1,kz) = 10.01E-6
             t3(ix,1,kz) = 25.E-6
             t4(ix,1,kz) = 50.e-6
     ...
     &          ,an=an,dn=dn1 )
        ...
        DO kz = kts,kte
          DO ix = its,ite
             re_cloud(kz,ix,jy) = MAX(2.51E-6, MIN(t1(ix,1,kz), 50.E-6))
             re_ice(kz,ix,jy)   = MAX(10.01E-6, MIN(t2(ix,1,kz), 125.E-6))
             re_snow(kz,ix,jy)  = MAX(25.E-6, MIN(t3(ix,1,kz), 999.E-6))
             ! check for case where snow needs to be treated as cloud ice (for rrtmg radiation)
             IF ( .not. present(qi) ) re_ice(kz,ix,jy)  = MAX(10.E-6, MIN(t3(ix,1,kz), 125.E-6))
          ENDDO
         ENDDO
       ...
       IF ( present(has_reqr) .and. present( re_rain ) ) THEN
         ...
         DO kz = kts,kte
           DO ix = its,ite
            re_rain(kz,ix,jy)  = MAX(50.E-6, MIN(t4(ix,1,kz), 2999.E-6))
           ENDDO
         ENDDO
         ENDIF
       ENDIF
       ...
       IF ( present(has_reqg) .and. present( re_graup ) ) THEN
         ...
         DO kz = kts,kte
           DO ix = its,ite
            re_graup(kz,ix,jy)  = MAX(50.E-6, MIN(t5(ix,1,kz), 10.E-3))
           ENDDO
         ENDDO
         ENDIF
       ENDIF
       ...
       IF ( present(has_reqh) .and. present( re_hail ) ) THEN
         ...
         DO kz = kts,kte
           DO ix = its,ite
            re_hail(kz,ix,jy)  = MAX(50.E-6, MIN(t5(ix,1,kz), 40.E-3))
           ENDDO
         ENDDO
         ENDIF
       ENDIF
   ...
   ! copy 2D slabs back to 3D
       ...
       DO kz = kts,kte
        DO ix = its,ite
         ...
         th(kz,ix,jy)  = an(ix,1,kz,lt)

         qv(kz,ix,jy)  = an(ix,1,kz,lv)
         qc(kz,ix,jy)  = an(ix,1,kz,lc)
         qr(kz,ix,jy)  = an(ix,1,kz,lr)
         IF ( flag_qi ) qi(kz,ix,jy)  = an(ix,1,kz,li)
         qs(kz,ix,jy)  = an(ix,1,kz,ls)
         qh(kz,ix,jy)  = an(ix,1,kz,lh)
         IF ( lhl > 1 ) qhl(kz,ix,jy) = an(ix,1,kz,lhl)
         
         IF ( lccn > 1 .and. is_aerosol_aware .and. flag_qnwfa ) THEN
           ! not used here
         ELSEIF ( flag_ccn .and. lccn > 1 .and. .not. flag_qndrop) THEN
            IF ( lccna > 1 .and. .not. ( present( cna ) .and. f_cnatmp ) ) THEN
              cn(kz,ix,jy) = Max(0.0, an(ix,1,kz,lccna) )
            ELSE
              cn(kz,ix,jy) = an(ix,1,kz,lccn)
            ENDIF
         ENDIF
         IF ( lccna > 1 ) THEN
           IF ( present( cna ) .and. f_cnatmp ) THEN
              cna(kz,ix,jy) = Max(0.0, an(ix,1,kz,lccna) )
           ENDIF
         ENDIF

         IF ( lcina > 1 ) THEN
           IF ( present( cni ) .and. f_cinatmp ) THEN
              cni(kz,ix,jy) = Max(0.0, an(ix,1,kz,lcina) )
           ENDIF
         ENDIF
         ...
         IF ( lccnuf > 0 .and. flag_cnuf ) THEN
           ...
           IF ( decayufccn ) THEN
             ...
             ENDIF
           ENDIF
           cnuf(kz,ix,jy) = an(ix,1,kz,lccnuf)
         ENDIF
         ...
         IF ( ipconc >= 5 ) THEN
          ...
          ccw(kz,ix,jy) = an(ix,1,kz,lnc)
          crw(kz,ix,jy) = an(ix,1,kz,lnr)
          IF ( present( cci ) ) cci(kz,ix,jy) = an(ix,1,kz,lni)
          csw(kz,ix,jy) = an(ix,1,kz,lns)
          chw(kz,ix,jy) = an(ix,1,kz,lnh)
          IF ( lhl > 1 ) chl(kz,ix,jy) = an(ix,1,kz,lnhl)
         ENDIF

         IF ( ipconc >= 6 ) THEN
            IF ( lzr > 0 )  zrw(kz,ix,jy) = an(ix,1,kz,lzr) *zscaleinv
            IF ( lzh > 0 )  zhw(kz,ix,jy) = an(ix,1,kz,lzh) *zscaleinv
            IF ( lzhl > 0 ) zhl(kz,ix,jy) = an(ix,1,kz,lzhl)*zscaleinv
         ENDIF
         ...
         IF ( has_wetscav ) THEN
           ...
           ENDIF
           IF ( PRESENT( rainprod ) ) rainprod(kz,ix,jy) = rainprod2d(ix,kz)
           IF ( PRESENT( evapprod ) ) evapprod(kz,ix,jy) = evapprod2d(ix,kz)
         ENDIF
#endif
